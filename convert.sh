...
sed -i'bkup' s/"<span lang='non' class='Unicode'>"/\<span\>\(\(\(non-/g preconversion.html
...
sed -i'bkup' s/^\\\\chapter/\\\\title/g converted.latex
sed -i'bkup' s/^\\\\section/\\\\chapter/g converted.latex
sed -i'bkup' s/^\\\\subsection/\\\\author/g converted.latex
sed -i'bkup' s/^\\\\subsubsection/\\\\section/g converted.latex
sed -i'bkup' s/^\\\\paragraph/\\\\subsection/g converted.latex
sed -i'bkup' s/^\\\\subparagraph/\\\\subsubsection/g converted.latex

# scrape out the text encoding for <span lang=> that we created in the
# first step and replace it with a Latex /textlang command
sed -i'bkup' s/"{(((non-"/\\\\texticelandic{/g converted.latex
